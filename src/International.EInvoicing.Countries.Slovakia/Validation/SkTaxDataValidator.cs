...
namespace International.EInvoicing.Countries.Slovakia.Validation;

/// <summary>
/// Judges a tax data document by OpenPeppol's own rules.
/// </summary>
/// <remarks>
...
/// </remarks>
public sealed class SkTaxDataValidator
{
    private readonly SchematronRuleSet _rules;

    private SkTaxDataValidator(SchematronRuleSet rules) => _rules = rules;
    ...
    public static SkTaxDataValidator LoadFrom(string directory)
    {
        ...
        if (!Directory.Exists(directory))
        {
            throw new DirectoryNotFoundException(
                $"No Slovak tax data rules at '{directory}'. They are not redistributable, so this library "
        ...
        return new SkTaxDataValidator(CompiledSchematron.Read(
