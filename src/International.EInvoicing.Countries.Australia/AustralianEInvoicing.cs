...
/// <summary>
/// Everything Australian, from one object.
/// </summary>
/// <remarks>
/// <para>
/// Australia exchanges <b>Peppol PINT</b>, not BIS Billing — it is outside the European family, and the two
...
[System.Diagnostics.CodeAnalysis.SuppressMessage(
    ...
    Justification = "Everything Australian hangs off one object on purpose; a static member here would send "
        + "the caller back to remembering which type builds what, which is the problem this type solves.")]
public sealed class AustralianEInvoicing
{
    private readonly EInvoicing _library;

    private AustralianEInvoicing(EInvoicing library) => _library = library;

    /// <summary>The whole library underneath, for anything this shortcut does not cover.</summary>
    public EInvoicing Library => _library;

    /// <summary>An Australian library instance: the Peppol profiles Australia exchanges.</summary>
    public static AustralianEInvoicing Create() => Create(pdf: null);

    /// <summary>The same, able to open hybrid PDFs.</summary>
    public static AustralianEInvoicing Create(IPdfAttachmentReader? pdf) =>
        Create(library => library.AddDefaults().AddAustralia(), pdf);

    /// <summary>The same, with anything else you want registered — the Peppol rules above all.</summary>
    /// <exception cref="ArgumentNullException"><paramref name="configure"/> is <c>null</c>.</exception>
    public static AustralianEInvoicing Create(Action<EInvoicingBuilder> configure) => Create(configure, null);

    /// <summary>The same, able to open hybrid PDFs.</summary>
    /// <exception cref="ArgumentNullException"><paramref name="configure"/> is <c>null</c>.</exception>
    public static AustralianEInvoicing Create(Action<EInvoicingBuilder> configure, IPdfAttachmentReader? pdf)
    {
        ArgumentNullException.ThrowIfNull(configure);

        return new AustralianEInvoicing(EInvoicing.Create(configure, pdf));
    }
    ...
    public static AustralianEInvoicing Over(EInvoicing library)
    {
        ArgumentNullException.ThrowIfNull(library);

        return new AustralianEInvoicing(library);
    }
    ...
    /// <summary>
    /// An Australian party, identified by its ABN where Peppol looks for it.
    /// </summary>
    ...
    /// <param name="identifier">The ABN, however it is spaced.</param>
    ...
    public PartyBuilder Describe(PartyBuilder party, string identifier, string name)
    {
        ...
        AuAbn abn = AuAbn.Parse(identifier);

        return party
            .Named(name)
            .WithLegalRegistration(abn.Value)
            .WithElectronicAddress(abn.Value, AuAbn.Scheme);
    }
