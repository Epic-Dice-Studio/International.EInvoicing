...
| 2026-09-03 | ZUGFeRD-csharp `Profile.cs` | Lists `EReporting` (`urn.cpro.gouv.fr:1p0:ereporting`) and `XRechnung` beside MINIMUM and EXTENDED, as though both were Factur-X conformance levels. | **One was ours, one was theirs.** XRECHNUNG is a real conformance level and we were stamping such containers `EN 16931` — fixed, and a profile with no published level is now refused rather than guessed. EReporting is not a conformance level at all: it is a business process identifier on an invoice reported inside a flux 10 document, and their enumeration conflates the two because their model has one. Nothing to register here. |
| 2026-09-03 | OpenPEPPOL advanced ordering scenarios | `OrderResponse_sc2.xml` uses `cac:OrderChangeDocumentReference`, which **UBL 2.1 does not define** — Peppol's advanced ordering is built on a later UBL than the schemas this library embeds. The document does not validate against 2.1 as published, before anything here touches it. | Changed what the round-trip test claims for those documents: **a round trip introduces no schema error the document did not already have**, rather than "no errors". The weaker-sounding assertion is the true one, holds for every document, and is what a caller needs to know — the alternative was excluding the document or pretending it was clean. Recorded because the same will happen to any post-award document Peppol moves onto a newer UBL. |
