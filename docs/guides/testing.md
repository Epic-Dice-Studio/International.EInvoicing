...
You have added a profile of your own, or a rule your largest customer insists on, or a reader for a dialect
of UBL one partner sends. Now you have to prove it works — and the hard part is not writing the assertions,
it is building a document the base norm accepts so that a failure means something.

This package is the kit: sample documents that conform, a round-trip harness that proves nothing was lost, a
corpus of documents that fight back, and assertions that read like the promise they defend.
...
The library's own test suite is organised around five kinds of test, and this package exists so yours can be
too:

1. **Parse** — the document reads, and the business terms you care about are where you expect them.
2. **Round trip** — `RoundTrip.Check`, and `Expect.LostNothing`.
3. **Conformance** — everything you write passes the official artefact: `Expect.Conforming`.
4. **Rules** — one test per rule, a passing case *and* a failing one: `Expect.Passed`, `Expect.Failed`.
5. **Diagnostics** — one test per code you emit: `Expect.Reported`.

See [validating a document](validation.md) for how to register the rule set that will judge you, and
[the recipes](../recipes/add-a-profile.md) for the shape of a profile of your own.
