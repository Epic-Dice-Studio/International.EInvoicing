...
| **Anchor extension data where it was read from** | An unmapped element is kept verbatim and written back at the end of its node, which UBL does not allow. Every case in the official corpora turned out to be a **term the reader did not map**, and mapping them left nothing to misplace — the despatch advice went the same way in September 2026: five of its six published documents map completely, and the sixth keeps one element that happens to sit where the schema still accepts it. What remains is genuinely foreign content — a national extension nobody here models — for which the anchor is still the right answer. **Sized, September 2026:** the fix is to record which sibling an extension followed and flush it there, which every writer must do at every element it writes — 163 call sites in the UBL invoice writer alone. Doing it for some writers and not others would leave the behaviour divergent, so it is a change to all of them at once, and it needs a session of its own rather than the tail of one. |
...
| **The Peppol documents that are not invoices** 🚧 *eight of nine transactions done, September 2026* | ✅ The **Order**, the **Despatch Advice**, the **Invoice Response** and the **Message Level Response** are read, written, schema-checked and judged by Peppol's own rules — measured on every document OpenPEPPOL publishes for them, with nothing left unmapped except one element on a carrier that identifies the driver. Together they are the chain an invoice sits at the end of: what was asked for, what was sent, what happened to the bill. The two responses are a UBL `ApplicationResponse` and fill `LifecycleStatusMessage`, the model the French CDAR messages already filled; the order and the despatch advice are their own documents with their own items, because an invoice's item is what is being charged for, a despatched item is a thing in a box, and an ordered item is a thing in a catalogue. See [the standard page](standards/peppol-post-award.md). The **Order Response** completes the pair an integration needs first: a seller may accept a line on other terms — a different quantity, a later date, a substitute product — and the buyer has to see which before the goods arrive. The **Order Cancellation** and the **advanced Order Response** followed; the advanced one needed no reader, being the same document under another profile. The **Order Agreement** — the response restating the whole order as the parties settled it — is mapped in full, certificates and all, because an element of an agreement left unmapped is a term of a contract nobody can see. **Still to do:** **Order Change**. Reading these also found where the shipped schemas end: `cac:OrderChangeDocumentReference` is not in UBL 2.1, so those documents are judged by whether a round trip makes them worse rather than by whether they are clean. |
...
0. **Close the gaps** — in this order, and before any new country:
   ...
   2. **The Peppol documents that are not invoices** 🚧 — ~~`ApplicationResponse`~~ ✅, ~~Despatch
      Advice~~ ✅, ~~Order~~ ✅, ~~Order Response~~ ✅, ~~Order Cancellation~~ ✅, the ~~advanced
      response~~ ✅ and the ~~Order Agreement~~ ✅ done. Only Order Change remains.
