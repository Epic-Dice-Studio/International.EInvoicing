...
| **The Peppol documents that are not invoices** 🚧 *responses done, September 2026* | ✅ The **Invoice Response** and the **Message Level Response** are read, written, schema-checked and judged by Peppol's own rules — measured on all fifteen documents OpenPEPPOL publishes, with nothing left unmapped and the same elements coming back in the same places. Both are a UBL `ApplicationResponse` and both fill `LifecycleStatusMessage`, the model the French CDAR messages already filled: one semantic statement, two syntaxes, which is why the model moved to the canonical layer. See [the standard page](standards/peppol-response.md). **Still to do:** **Ordering** and **Despatch Advice**, which are different documents with different roots and their own models rather than the same one under another profile.
...
0. **Close the gaps** — in this order, and before any new country:
   ...
   2. **The Peppol documents that are not invoices** 🚧 — ~~`ApplicationResponse`~~ ✅ done: the Invoice
      Response and the Message Level Response, which is what a Peppol receiver owes a sender. Ordering and
      Despatch Advice next; they are different documents, not the same one under another profile.
