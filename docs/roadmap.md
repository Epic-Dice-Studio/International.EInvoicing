...
| **The Peppol documents that are not invoices** 🚧 *responses and despatch advice done, September 2026* | ✅ The **Invoice Response**, the **Message Level Response** and the **Despatch Advice** are read, written, schema-checked and judged by Peppol's own rules — measured on every document OpenPEPPOL publishes for them, with one element in the whole corpus left unmapped and reported. The two responses are a UBL `ApplicationResponse` and fill `LifecycleStatusMessage`, the model the French CDAR messages already filled; the despatch advice is its own document and its own item, because an invoice's item is what is being charged for and a despatched item is a physical thing in a box. See [the standard page](standards/peppol-post-award.md). **Still to do:** **Ordering** — the Order, Order Response, Order Change, Order Cancellation and Order Agreement, five transactions with their own models. |
...
0. **Close the gaps** — in this order, and before any new country:
   ...
   2. **The Peppol documents that are not invoices** 🚧 — ~~`ApplicationResponse`~~ ✅ and ~~Despatch
      Advice~~ ✅ done. **Ordering** is what is left: five transactions with their own models, and the
      biggest single piece remaining in this list.
