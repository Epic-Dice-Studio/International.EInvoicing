...
| **Anchor extension data where it was read from** | The defect above, and the next reliability pass. An unmapped element is kept verbatim and written back at the end of its node; it has to go back where it came from. That is a change to the reader, the model and both writers, which is why it is its own item rather than a footnote. Pinned meanwhile by a test that allows exactly this failure and nothing else. |
...
0. **Close the gaps** — in this order, and before any new country:
   ...
   1a. **Anchor extension data where it was read from**, so a document read and written back keeps a shape a
      receiver accepts.
