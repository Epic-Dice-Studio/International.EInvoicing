...
| **Anchor extension data where it was read from** | An unmapped element is kept verbatim and written back at the end of its node, which UBL does not allow. This looked like the next big reliability item and shrank to a small one: every case in the official corpus turned out to be a **term the reader did not map**, and mapping them left nothing to misplace. What remains is genuinely foreign content — a national extension nobody here models — for which the anchor is still the right answer. |
...
| **The Factur-X metadata we write is not the document's own** | Found by building the check below. PDFsharp generates its own XMP while saving and puts it in the catalogue's `/Metadata` whatever was there, so the Factur-X block this library writes ends up in the file as an object nothing points at: a receiver reading the document's metadata, which is what the specification tells it to do, sees a producer string and no profile. **Every hybrid PDF this library writes is affected.** Three ways out, in order of preference: append the metadata as a PDF *incremental update* after the save, which is standard and leaves every existing byte offset alone; get the backend to stop overwriting; or write a second `IPdfAttachmentWriter` on another PDF library. Pinned by a test that fails the day it is fixed. |
...
| **Getting the readable copy, and the supporting documents, out** | An invoice carries things a person opens: the **readable rendition** — for a hybrid invoice, the PDF the XML arrived in — and the **supporting documents** of BG-24, which may be embedded bytes (BT-125) or a URI to fetch (BT-124). The model holds all of it today, and getting at it reads like plumbing: `invoice.AdditionalDocuments.Where(d => d.Attachment.IsSet)`, then bytes, then a `MemoryStream`. It should be one call that hands back a stream, its media type and its file name — and, for a document read from a PDF, the container itself, which is currently dropped once the XML is out of it. The precise shape: a rendition is *the invoice, readable*; a supporting document is *something else, attached*; a reference with a URI is neither, since fetching it is network I/O this library does not do. Naming them apart matters more than the convenience, because a caller who treats a delivery note as the invoice's readable copy has mixed up two different things. |
...
| **The cases their corpora exercise and ours did not** | Reading the neighbours' `test/resources` folders cost an hour and found two defects — every VAT category but standard was unwritten and untested, and two bank accounts on one invoice produced a shape no schema accepts. Both are fixed. What is left from that list: **sub-invoice lines** (Factur-X EXTENDED nests them, EN 16931 does not), the **`XRECHNUNG` and `EREPORTING` Factur-X profiles** we do not register, and **PDF containers that fight back** — password-protected, no embedded file, too small to be a PDF, XMP as attributes, XMP that does not parse — which the hostile corpus does not cover because it covers XML only. See [prior art](prior-art.md). |
...
0. **Close the gaps** — in this order, and before any new country:
   ...
   3. ~~**The Factur-X container checks that are ours**~~ ✅ done for incoming documents — and it found that
      **our own** container carries PDFsharp's metadata rather than the Factur-X block, which is now the
      first item in the table above.
   3a. **Getting the readable copy and the supporting documents out** in one call, and keeping the PDF a
      hybrid invoice arrived in instead of dropping it.
